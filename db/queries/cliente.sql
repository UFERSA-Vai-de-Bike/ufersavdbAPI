...
BEGIN
	RETURN QUERY SELECT username FROM client_vdb;
END;
$$ LANGUAGE plpgsql;
...
RETURNS TABLE(id client_vdb.idCli%TYPE,username client_vdb.username%TYPE) AS $$
BEGIN
	RETURN QUERY SELECT idCli, username FROM client_vdb WHERE state = TRUE AND onBike = FALSE;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
	...
	SELECT idCli INTO id_temp FROM client_vdb WHERE username = $2;
